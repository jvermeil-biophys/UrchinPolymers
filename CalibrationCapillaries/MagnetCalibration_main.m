...
function popt = fitPowerLaw_tr(X, Y, p0, lb, ub)
...
function popt = fitPowerLaw_lm(X, Y, p0)
    ft = fittype('A*x^k','independent','x','coefficients',{'A','k'});
    opts = fitoptions(ft);
...
function popt = fitDoubleExpo_tr(X, Y, p0, lb, ub)
...
function popt = fitDoubleExpo_lm(X, Y, p0)
    ft = fittype('A*exp(-x/k1) + B*exp(-x/k2)', ...
                 'independent','x', ...
                 'coefficients',{'A','k1','B','k2'});
    opts = fitoptions(ft);
...
function tracks_data = tracks_pretreatment(all_tracks, SCALE, FPS, ...
...
function tracks_analysis(tracks_data, expLabel, saveResults, savePlots, saveDir)
    MagR = 60; % µm
    % Colors
    c_V = hex2rgb('#1f77b4'); % Strong blue
    c_F = hex2rgb('#2ca02c'); % Dark lime green
    c_excluded = hex2rgb('#ff7f0e'); % Vivid orange
    c_fit2exp = hex2rgb('#ff0000'); % Red
    c_fitpL = hex2rgb('#ff8c00'); % Dark orange
    ...
    for i=1:length(tracks_data)
        ...
        if (crit1 && crit2) || bypass1
            tracks_data_f1{end+1} = track; 
        end
    end

    fig1 = figure('Name','Trajectories');
    fig1.Position = [100, 100, 1500, 500];
    ax1 = subplot(1,3,1);
    title(ax1, sprintf('All tracks, N = %d', length(tracks_data)));
    hold(ax1,'on');
    for i=1:length(tracks_data)
        plot(ax1, tracks_data{i}.X, tracks_data{i}.Y, 'LineWidth',1.1);
    end
    ...
    for i=1:length(tracks_data_f1)
        plot(ax2, tracks_data_f1{i}.X, tracks_data_f1{i}.Y, 'LineWidth',1.1);
    end
    viscircles([0,0], MagR, 'Color',[0.5 0.5 0.5]);
    ...
    V_popt_pL = fitPowerLaw_lm(all_D, all_V, p0); % lb, ub
    ...
    a = 0.05;
    scatter(ax3, all_D, all_V, 10, c_V, 'filled', 'Marker','o', 'MarkerFaceAlpha',a);
    scatter(ax3, [], [], 10, c_V, 'filled', 'Marker','o', 'MarkerFaceAlpha', 1);
    ...
    scatter(ax3, all_removedD, all_removedV, 10, c_excluded, 'filled', 'Marker','o', 'MarkerFaceAlpha', a);
    MD = max(all_D); MV = max(all_V);
    xlim(ax3,[0,1.1*MD]); ylim(ax3,[0,1.2*MV]); grid(ax3,'on');
    legend(ax3,'','Data','Naive fit',sprintf('High cut, %.2f', high_cut), sprintf('Low cut, %.2f', low_cut));
    ...
    V_popt_2exp = fitDoubleExpo_lm(all_D, all_V, p0); % lb, ub
    V_fit_2exp = doubleExpo(D_plot, V_popt_2exp(1), V_popt_2exp(2), V_popt_2exp(3), V_popt_2exp(4));

    F_popt_2exp = fitDoubleExpo_lm(all_D, all_F, p0); % lb, ub
    ...
    ax = nexttile([1 2]); hold(ax,'on');
    scatter(ax, all_D, all_V, 10, c_V, 'filled', 'Marker','o','MarkerFaceAlpha',a); 
    scatter(ax, [], [], 10, c_V, 'filled', 'Marker','o', 'MarkerFaceAlpha',1); hold(ax,'on');
    plot(ax, D_plot, V_fit_2exp, '-', 'Color',c_fit2exp, 'LineWidth',1.5);
    plot(ax, D_plot, V_fit_pL, '-', 'Color',c_fitpL, 'LineWidth',1.5);
    ...
    % Plot F
    ax = nexttile([1 2]); hold(ax,'on');
    scatter(ax, all_D, all_F, 10, c_F, 'filled', 'Marker', 'o', 'MarkerFaceAlpha', a); 
    ...
    ax = nexttile([1 2]); hold(ax,'on');
    scatter(ax, all_D, all_F, 10, c_F, 'filled', 'Marker', 'o', 'MarkerFaceAlpha', a); 
    scatter(ax, [], [], 10, c_F, 'filled', 'Marker', 'o', 'MarkerFaceAlpha', 1); 
    plot(ax, D_plot, F_fit_2exp, '-', 'Color', c_fit2exp, 'LineWidth', 1.5);
    plot(ax, D_plot, F_fit_pL, '-', 'Color', c_fitpL, 'LineWidth', 1.5);
