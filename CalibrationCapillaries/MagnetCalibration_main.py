...
def tracks_analysis(tracks_data, expLabel = '', 
    ...
    #### Third filter (optionnal, exceptionnal)
    # filterD = (all_D >= 185)
    # all_D = all_D[filterD]
    # all_V = all_V[filterD]
    # all_F = all_F[filterD]
    ...
    alpha = min(1, max(0.01, 500/len(all_D)))
...
def compareCalibrations(srcDir, labelList = [], 
    ...
    for i, data in enumerate(dataList):
        ...
        # alpha = min(1, max(0.005, 500/len(all_D)))
        alpha = 0.2
    ...
    titles = ['Velocity', 'Force']
    for ax, title in zip([axes1[1], axes2[1]], titles):    
    ...
    for fig, title in zip([fig1, fig2], titles):
        fig.suptitle(title + ' ' + MainTitle, fontsize=12)
        fig.tight_layout()
    
    plt.show()
    
    if savePlots:
        fig1.savefig(os.path.join(saveDir, 'CompareV_' + saveTitle + '.png'), dpi=500)
