...
def getGlycerolViscosity(ratio, T):
    ...
    waterDen = (1-np.power(((np.abs(T-4))/622),1.7)) 	#Density of water (g/cm3)
    ...
    contraction_av = 1 - np.power(3.520E-8*((mass_fraction*100)),3) + \
                     np.power(1.027E-6*((mass_fraction*100)),2) + \
                     2.5e-4*(mass_fraction*100)-1.691E-4
    ...
    alpha = 1 - mass_fraction + \
            (a*b*mass_fraction*(1 - mass_fraction)) / \
            (a*mass_fraction + b*(1 - mass_fraction))
    A     = np.log(waterVisc / glycerolVisc)

    viscosity_mix = glycerolVisc * np.exp(A*alpha) * 1000 # in mPas.s
...
def getGlycerolViscosity_original(ratio, T):
    T = T 			       	#temperature (degrees Celcius)
    waterVol = 1 - ratio	#volume of water required (ml)
    glycerolVol = ratio		#volume of Glycerol used (ml)
